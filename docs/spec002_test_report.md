...
Result: **38 passed, 0 failed, 0 pending** (across the 21 required tests plus
TEST 22, added by GPT Review #002 Round 1/PATCH #002-A, TEST 23, added by
PATCH #001-C, and TEST 24, added by PATCH #002-B -- several have multiple
focused sub-tests). Full repo (Spec #001 + Spec #002): **65 passed, 1
...
| 24 | Pre-budget observation isolation | `test_24_pre_budget_observation_isolation.py` | **PASS** | Added by PATCH #002-B (Spec #003 IMPLEMENTATION BLOCKER §74A). `compute_discovery_observations()`'s output is byte-identical across default/tiny/disabled `candidate_budget` configs, while `run_discovery()`'s post-budget output genuinely changes size -- proves the pre-budget statistical layer is isolated from the budget knob, not merely untested against it. |
