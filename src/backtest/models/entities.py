...
def calendar_fingerprint(
    ...
    dates_part = ",".join(sorted(session_dates))
    early_part = ",".join(f"{d}:{t}" for d, t in sorted(early_close_dates))
    return (
        f"{source}::{calendar_identifier}::{calendar_version}::{market}::{timezone}::"
        f"{coverage_start}::{coverage_end}::{dates_part}::{session_open_time}::"
        f"{session_close_time}::{early_part}"
    )
...
def execution_semantics_fingerprint(
    ...
    return f"{entry_fill}::{invalidation_detection}::{invalidation_fill}::{time_exit_fill}::{cap_fill}::{cap_is_hard}"
...
def research_plan_fingerprint(
    ...
    folds_part = ",".join(f"{f.fold_id}:{f.start_date}:{f.end_date}" for f in selection_folds)
    cohort_part = ",".join(sorted(hypothesis_cohort_ids))
    rule_part = (
        f"{selection_rule.minimum_executed_trades}:{selection_rule.minimum_evaluable_trades}:"
        f"{selection_rule.minimum_evaluable_ratio}:{selection_rule.ranking_metric}:"
        f"{selection_rule.minimum_selection_metric}:{selection_rule.threshold_operator}:{selection_rule.tie_break}"
    )
    costs_part = (
        f"{cost_assumptions.commission_entry_rate}:{cost_assumptions.commission_exit_rate}:"
        f"{cost_assumptions.slippage_entry_bps}:{cost_assumptions.slippage_exit_bps}:"
        f"{cost_assumptions.borrow_annual_rate}:{cost_assumptions.borrow_accrual_convention}:{cost_assumptions.label}"
    )
    exposure_part = (
        f"{','.join(sorted(exposure_manifest.prior_validation_disclosures))}:{exposure_manifest.declared_unseen}"
    )
    return (
        f"{formation_start}::{formation_end}::{validation_start}::{validation_end}::{locked_oos_start}::"
        f"{folds_part}::{cohort_part}::{trading_calendar_id}::{benchmark_security_id}::"
        f"{execution_semantics_profile_id}::{rule_part}::{costs_part}::{exposure_part}"
    )
