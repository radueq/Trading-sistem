...
Result is keyed by record_key() (signature_id, timeframe, horizon_bars,
outcome_type, evaluation_run_id) -- NOT by signature_id alone, since the
same signature is tested at multiple horizons in a real run (PATCH
#003-A, GPT Review #003 Round 1 finding #1) and a bare signature_id key
would let one horizon's result silently overwrite another's.
"""
import pytest
...
def test_bh_matches_hand_computed_values():
    ...
    assert result[record_key(records[0])][0] == pytest.approx(0.004)
    assert result[record_key(records[1])][0] == pytest.approx(0.04)
    assert result[record_key(records[2])][0] == pytest.approx(0.6667, abs=1e-4)
    assert result[record_key(records[3])][0] == pytest.approx(0.8)
    for key in result:
        assert result[key][1] == "1D|3bars|relative_return|run1"
...
def test_bh_adjusted_p_is_monotone_non_decreasing_with_rank():
    ...
    adj_in_rank_order = [result[record_key(r)][0] for r in ranked]
