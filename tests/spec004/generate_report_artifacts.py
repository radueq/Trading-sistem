...
-> preregister_hypothesis() atomic gate -> registry), run against
hand-built #003-shaped EvidenceProfiles -- nothing here is hand-typed
output.

PATCH #004-A (GPT Review #004 Round 1): this script now goes through
`registry.preregistration.preregister_hypothesis()` -- the ONE atomic
entry point that may introduce a PREREGISTERED StrategyHypothesis --
instead of hand-building an already-PREREGISTERED object and calling
the now-generic `HypothesisRegistry.register()` directly (finding #1).
`build_evidence_packet()` no longer takes a free `primary_horizon_bars`
parameter (finding #4) and every profile list below covers the FULL set
of horizons the example's `EvaluationRunRegistry` claims to have tested
(finding #3).
...
from hypothesis.models.entities import (
    AgentReview, ComplexityStatus, HypothesisComplexitySnapshot, HypothesisProvenance,
...
def _build_and_preregister(raw_proposal: dict, registry: HypothesisRegistry, run_reg: "eval_entities.EvaluationRunRegistry"):
    """Runs the FULL real pipeline: normalize -> validate -> register
    proposal -> consensus -> human APPROVE decision -> build draft
    hypothesis -> materialize variants -> the ONE atomic
    `preregister_hypothesis()` gate. Returns (proposal, validation,
    hypothesis_or_None, variants, error_strings)."""
    ...
    draft = draft.__class__(**{**draft.__dict__, "variant_ids": tuple(v.strategy_variant_id for v in variants)})
    ...
    try:
        ...
        registry.mark_proposal_rejected(proposal.proposal_id)
        return proposal, validation, None, variants, (str(exc),)
...
def _example_a() -> str:
    ...
    profiles = [
        ...
        make_evidence_profile(10, mean_relative=0.001, adjusted_p=0.55),
    ]
    ...
    raw = make_proposal_raw(proposal_id="prop_A", horizon_candidates={
        "unit": "BARS", "values": [1, 2, 3], "selection_basis": "decay strongest at 2-3 bars, still positive at 1",
        "parameter_source": "EVIDENCE_DERIVED",
    })
    proposal, validation, hyp, variants, errors = _build_and_preregister(raw, registry, run_reg)
    ...
    lines.append("Preregistered through the ONE atomic gate (registry.preregistration.preregister_hypothesis()) -- "
                  "never a hand-built PREREGISTERED object handed to register() (PATCH #004-A finding #1).")
    lines.append("")
    return "\n".join(lines)
...
def _example_b() -> str:
    ...
    raw_long = make_proposal_raw(
        ...
        horizon_candidates={"unit": "BARS", "values": [2, 3], "selection_basis": "unchanged despite weak evidence -- direction is not re-derived", "parameter_source": "PRE_SPECIFIED"},
    )
    _, val_long, hyp_long, _, _ = _build_and_preregister(raw_long, registry, run_reg)
    ...
    raw_short = make_proposal_raw(
        ...
        horizon_candidates={"unit": "BARS", "values": [2, 3], "selection_basis": "same evidence, opposite direction, explicitly proposed on its own", "parameter_source": "PRE_SPECIFIED"},
    )
    _, val_short, hyp_short, _, _ = _build_and_preregister(raw_short, registry, run_reg)
...
def _example_c() -> str:
    ...
    raw = make_proposal_raw(
        proposal_id="prop_C", horizon_candidates={
            "unit": "BARS", "values": [2, 3, 5], "selection_basis": "decay concentrated in the 2-5 bar zone -- ALL THREE kept as candidates, none pre-selected",
            "parameter_source": "EVIDENCE_DERIVED",
        },
    )
    _, validation, hyp, variants, _ = _build_and_preregister(raw, registry, run_reg)
    ...
    lines.append("No variant is auto-tagged BASELINE_VARIANT by `min(horizon)` either (PATCH #004-A finding #5, TEST 60): "
                  f"variant_tags here are {sorted({v.variant_tag for v in time_exit_variants})} -- all EXPERIMENTAL_VARIANT "
                  "unless a baseline is explicitly pre-designated via `baseline_time_exit_bars`.")
    lines.append("No field named selected_horizon/optimal_horizon exists anywhere on StrategyHypothesis: confirmed structurally by TEST 12.")
    lines.append("")
    return "\n".join(lines)
